...
The explicit `logs` backend does not use the network and does not store credentials. The default `auto` source may use the live Codex API backend, which reads the Codex CLI auth file and refreshes tokens in that same Codex-owned file; this package does not create a separate credential store.
